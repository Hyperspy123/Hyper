  ...
  return (
    <>
      <header className="p-5 flex justify-between items-center bg-[#0a0f3c] border-b border-white/5 sticky top-0 z-50 backdrop-blur-lg" dir="rtl">
        {/* Menu Toggle Button - Now on the Left */}
        <button 
          onClick={() => setIsMenuOpen(true)}
          className="p-2.5 bg-white/5 rounded-xl border border-white/10 text-cyan-400 active:scale-95 transition-all"
        >
          <Menu size={24} />
        </button>
      ...
      {/* Side Menu Drawer - Anchored Left, Sliding from Left */}
      <div 
        className={`fixed top-0 left-0 h-full w-[280px] bg-[#0d1435] z-[70] shadow-2xl transition-transform duration-300 ease-in-out border-r border-white/5 ${
          ...
          <nav className="space-y-3 flex-1">
            {menuItems.map((item) => (
              ...
              >
                ...
                <ChevronRight size={16} className="text-gray-600 group-hover:text-cyan-400 rotate-180" />
              </button>
            ))}
          </nav>
