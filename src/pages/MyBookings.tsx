...
export default function BookCourt() {
  ...
  // Generate next 7 days for the easy-tap scroller
  const getNextSevenDays = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push({
        ...
        dayName: i === 0 ? 'اليوم' : daysOfWeek[d.getDay()],
        dayNum: d.getDate()
      });
    }
    return days;
  ...
  if (loading) return <div className="min-h-screen bg-[#0a0f3c] flex items-center justify-center text-cyan-500 font-bold">جاري تحميل الملعب...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#0a0f3c] text-white font-sans pb-10" dir="rtl">
      ...
      <div className="p-4 flex items-center gap-4">
        ...
        <h1 className="text-2xl font-black italic tracking-tight">تأكيد الحجز</h1>
      </div>
      ...
      <main className="p-6 max-w-lg mx-auto space-y-8">
          ...
          <div className="flex gap-2">
            {durations.map((d) => (
              <button 
                ...
                className={`flex-1 py-3 rounded-xl font-bold text-xs transition-all border ${duration === d.value ? 'bg-cyan-500 border-cyan-500 text-[#0a0f3c]' : 'bg-[#14224d] border-white/5 text-gray-400'}`}
              >
                {d.label}
              </button>
            ))}
        ...
        {/* 2. New Easy Date Picker (Scrollable Days) */}
        <section className="space-y-4">
          <label className="text-gray-400 text-sm font-bold flex items-center gap-2">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {getNextSevenDays().map((day) => (
              <button
                ...
                className={`flex flex-col items-center min-w-[75px] py-4 rounded-2xl transition-all border ${
