...
export default function Faz3a() {
  ...
  // 2. دالة جلب البيانات (محسنة لفرز التبويبات بدقة)
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        `)
        .eq('status', 'open') // جلب الفزعات المفتوحة فقط
        .order('created_at', { ascending: false });

      if (error) throw error;

      // أ. الفزعات المتاحة: التي لم ينشئها المستخدم ولم ينضم إليها بعد
      ...
      // ب. فزعاتي: التي أنشأتها أنا أو انضممت إليها كـ "فزيع"
  ...
  // 🔥 تفعيل البث المباشر (Realtime) لتحديث العداد والأسماء فوراً
  useEffect(() => {
    fetchData();
      ...
      <main className="p-6 max-w-md mx-auto space-y-8 pt-24 text-right">
        ...
        <div className="space-y-6">
          {loading && (activeTab === 'public_faz3at' ? publicFaz3at.length === 0 : myFullFaz3at.length === 0) ? (
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-4 text-right">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[9px] font-black text-cyan-500/60 uppercase mt-1 italic tracking-tighter">{post.profiles?.current_rank || 'بطل هايب'}</p>
                      </div>
                    </div>
                    <div className="bg-cyan-500/10 text-cyan-400 px-3 py-1 rounded-full text-[9px] font-black italic border border-cyan-500/20">لايف 🔥</div>
                  </div>
                  ...
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/5 space-y-3">
                    <div className="flex justify-between items-center text-[10px] font-[1000] italic uppercase">
                      <span className={`${isFull ? 'text-green-400' : 'text-cyan-400'}`}>
                        {isFull ? 'مكتمل ✅' : `باقي ${post.missing_players - count} أبطال`}
                      </span>
                      <div className="flex items-center gap-2 text-gray-400"><Users size={12} /> حالة الفريق</div>
                    </div>
                    <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div className={`h-full transition-all duration-1000 ${isFull ? 'bg-green-500' : 'bg-cyan-500'}`} style={{ width: `${Math.min((count / post.missing_players) * 100, 100)}%` }} />
                    </div>
                  </div>
                ...
                <div key={post.id} className="bg-[#0a0f3c]/60 border border-cyan-500/20 rounded-[35px] p-7 space-y-6 shadow-2xl text-right animate-in fade-in">
                  ...
                  <div className="space-y-4">
                     ...
                     <div className="flex flex-col gap-3">
                        {/* إذا لست المالك، أظهر المالك (الراعي) أولاً */}
            ...
            }) : <p className="text-center opacity-20 py-20 italic font-black text-gray-500">لم تنشر أو تشارك في أي فزعة بعد</p>
          )}
        </div>
      </main>
    </div>
