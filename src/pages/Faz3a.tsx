...
import { MapPin, Clock, Zap, Plus, Loader2, CheckCircle2, X, Trash2 } from 'lucide-react';
...
export default function Faz3a() {
  ...
  // 2. دالة جلب البيانات (تستخدم Real-time أو تُستدعى عند تغيير التبويب)
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'community') {
        ...
        // جلب طلباتي أنا من جدول faz3a_posts
        ...
        // جلب طلبات الآخرين (الفزعة العامة)
        const { data } = await supabase.from('faz3a_posts').select('*, profiles(first_name, last_name, skill_level)').neq('creator_id', currentUserId).order('created_at', { ascending: false });
  ...
  // 3. دالة إلغاء الفزعة (الحذف)
  const handleDeletePost = async (postId: string) => {
    try {
      const { error } = await supabase.from('faz3a_posts').delete().eq('id', postId);
      if (error) throw error;

      toast.success("تم إلغاء طلب الفزعة بنجاح");
      // تحديث القائمة محلياً فوراً
      setMyRequests(prev => prev.filter(p => p.id !== postId));
    } catch (error: any) {
      toast.error("فشل الإلغاء: " + error.message);
    }
  };

  // 4. دالة إنشاء طلب فزعة (يظهر في العامة وطلباتي)
  const handleCreatePost = async () => {
    ...
    try {
      ...
      setActiveTab('my_requests'); // الانتقال لرؤية الطلب في "طلباتي"
    } catch (error: any) {
      ...
      <main className="p-6 max-w-md mx-auto space-y-8 relative z-10 pt-24 text-right">
        ...
        <div className="space-y-4 min-h-[400px]">
          {loading ? (
            ...
            myRequests.map(post => (
              <div key={post.id} className="bg-white/5 border border-cyan-500/30 rounded-[35px] p-6 space-y-4 backdrop-blur-xl relative group">
                 <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black text-cyan-400 uppercase italic">طلبك نشط في العامة</span>
                    <button 
            ...
            othersRequests.map(post => (
                 ...
                 <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[8px] font-black text-gray-500 uppercase">{post.profiles?.skill_level}</p>
                      </div>
                    </div>
                    <div className="bg-cyan-500 text-[#0a0f3c] px-3 py-1 rounded-full text-[10px] font-black italic">ناقصنا {post.missing_players}</div>
                  </div>
             ...
             /* تبويب المجمع (نفس الكود السابق لإرسال الدعوات الخاصة) */
             communityPlayers.map(player => (
               <div key={player.id} className="bg-white/5 border border-white/10 rounded-[35px] p-6 flex items-center justify-between group">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-cyan-500/10 border border-white/10 flex items-center justify-center font-black text-cyan-400 uppercase">{player.first_name?.[0]}</div>
                    <div className="text-right">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#05081d]/90 backdrop-blur-md">
            <div className="bg-[#0a0f3c] border border-white/10 w-full max-w-sm rounded-[40px] p-8 space-y-6 animate-in zoom-in-95">
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-black italic text-cyan-400 tracking-tighter">إنشاء طلب فزعة</h3>
              ...
              <button 
                ...
                className="w-full py-5 bg-cyan-500 text-[#0a0f3c] rounded-[24px] font-black uppercase text-xs shadow-lg flex items-center justify-center gap-2"
              >
