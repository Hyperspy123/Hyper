...
export default function Faz3a() {
  ...
  const createPost = async (e: React.FormEvent) => {
    ...
    if (error) { 
      ...
      toast.success("تم عرض حجزك بنجاح 🚀"); 
      setShowCreate(false); 
      fetchData(); 
    }
  };
    ...
    try {
      ...
      if (success) {
        ...
        await supabase.from('notifications').insert([{ 
          ...
          message: `انضم لاعب لحجزك في ${post.court_name}. يمكنك التواصل معه الآن.`, 
          is_read: false 
        }]);
  ...
  return (
    <div className="min-h-screen bg-[#05081d] text-white font-sans pb-32 relative text-right" dir="rtl">
      ...
      <main className="p-6 max-w-md mx-auto space-y-8 pt-24 text-right">
        ...
        {showCreate && (
          <form onSubmit={createPost} className="bg-[#0a0f3c] border border-cyan-500/30 rounded-[35px] p-6 space-y-4 animate-in zoom-in-95 shadow-2xl">
            <h3 className="text-lg font-black italic text-cyan-400">عرض حجز للنقل 📝</h3>
            <input required placeholder="اسم الملعب..." className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl outline-none focus:border-cyan-500 text-right font-bold" onChange={e => setNewPost({...newPost, court_name: e.target.value})} />
            <div className="grid grid-cols-2 gap-4">
              <input required type="date" className="bg-white/5 border border-white/10 p-4 rounded-2xl outline-none text-right font-bold text-cyan-400" onChange={e => setNewPost({...newPost, match_date: e.target.value})} />
              <input required type="time" className="bg-white/5 border border-white/10 p-4 rounded-2xl outline-none text-right font-bold text-cyan-400" onChange={e => setNewPost({...newPost, match_time: e.target.value})} />
            </div>
            <input required placeholder="السعر المطلوب (ريال)" className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl outline-none focus:border-cyan-500 text-right font-bold" onChange={e => setNewPost({...newPost, price: e.target.value})} />
            <button className="w-full py-5 bg-cyan-500 text-[#0a0f3c] font-[1000] rounded-2xl uppercase shadow-lg shadow-cyan-500/30 transition-all active:scale-95">نشر الحجز 🚀</button>
          </form>
        )}
        ...
        <div className="space-y-6">
          {loading ? (
            ...
            publicFaz3at.length > 0 ? publicFaz3at.map(post => (
                <div key={post.id} className="bg-[#0a0f3c]/40 border border-white/10 rounded-[40px] p-7 space-y-5 backdrop-blur-2xl relative overflow-hidden group hover:border-cyan-500/30 transition-all shadow-xl">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-right">
                    <div className="bg-white/5 p-4 rounded-2xl text-[10px] font-black italic border border-white/5 flex items-center gap-2 truncate justify-end">{post.court_name} <MapPin size={14} className="text-cyan-400" /></div>
                    <div className="bg-white/5 p-4 rounded-2xl text-[10px] font-black italic border border-white/5 flex items-center gap-2 justify-end">{post.match_time} <Clock size={14} className="text-cyan-400" /></div>
                  </div>
            ...
            )) : <p className="text-center opacity-20 py-20 italic font-black uppercase tracking-widest text-gray-500">لا توجد حجوزات متاحة حالياً</p>
          ) : (
              ...
              return (
                <div key={post.id} className="bg-[#0a0f3c]/60 border border-cyan-500/20 rounded-[35px] p-7 space-y-6 shadow-2xl text-right">
                  <div className="flex justify-between items-center">
                     <div className="text-right">
                        <p className="text-[10px] font-black text-cyan-400 uppercase italic mb-1">{isOwner ? "حجزك المنشور" : "حجز انضممت له"} <CheckCircle2 size={12} className="inline mr-1" /></p>
            ...
            }) : <p className="text-center opacity-20 py-20 italic font-black uppercase tracking-widest text-gray-500">لم تقم بأي عملية نقل بعد</p>
          )}
        </div>
      </main>
    </div>
