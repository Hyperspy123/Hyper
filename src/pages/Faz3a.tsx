...
import { MapPin, Clock, Zap, Plus, Loader2, CheckCircle2, X, Trash2, User, ChevronLeft, Send } from 'lucide-react';
...
export default function Faz3a() {
  ...
  const [invitingId, setInvitingId] = useState<string | null>(null); // لتتبع حالة تحميل زر الدعوة
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'community') {
        const { data } = await supabase.from('profiles').select('*').eq('is_public', true).neq('id', currentUserId).order('total_matches', { ascending: false });
        ...
        const { data } = await supabase.from('faz3a_posts').select('*, profiles(first_name, last_name, skill_level, current_rank)').order('created_at', { ascending: false });
  ...
  // 🔥 دالة إرسال الدعوة من تبويب المجتمع
  const handleSendInvite = async (receiverId: string, receiverName: string) => {
    // نحتاج أولاً للتأكد أن المستخدم لديه طلب فزعة نشط ليدعو اللاعب إليه
    const { data: activeRequests } = await supabase.from('faz3a_posts').select('id').eq('creator_id', currentUserId).limit(1);

    if (!activeRequests || activeRequests.length === 0) {
      toast.error("يجب إنشاء طلب فزعة أولاً قبل دعوة اللاعبين!");
    ...
    try {
      ...
      toast.success(`تم إرسال دعوة فزعة لـ ${receiverName} 🎾🔥`);
    } catch (error) {
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success("تم نشر الفزعة! الحين تقدر تدعو لاعبين من قائمة المجتمع 🔥");
      setIsModalOpen(false);
      setActiveTab('community'); // توجيه المستخدم مباشرة للمجتمع ليدعو اللاعبين
    } catch (error: any) {
      ...
      <main className="p-6 max-w-md mx-auto space-y-8 relative z-10 pt-24">
        ...
        <div className="space-y-6 min-h-[400px]">
          {loading ? (
            ...
            othersRequests.map(post => {
              ...
              return (
                <div key={post.id} className={`relative bg-white/5 border rounded-[35px] p-7 space-y-5 backdrop-blur-2xl transition-all duration-500 shadow-2xl ${isMyPost ? 'border-cyan-500/50' : 'border-white/10'}`}>
                  {isMyPost && <div className="absolute -top-3 left-6 bg-cyan-500 text-[#0a0f3c] px-4 py-1.5 rounded-full text-[9px] font-[1000] uppercase italic tracking-tighter">طلبك</div>}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex items-center gap-3 bg-white/5 p-3.5 rounded-2xl text-[10px] font-black italic">
                  ...
                  {isMyPost ? (
                    <button onClick={() => handleDeletePost(post.id)} className="w-full py-4 bg-red-500/10 text-red-500 border border-red-500/10 rounded-[22px] font-black text-[10px] uppercase flex items-center justify-center gap-2 hover:bg-red-500 hover:text-white transition-all">
                      <Trash2 size={16} /> إلغاء الطلب
                    </button>
                  ) : (
                    <button className="w-full py-4 bg-white text-[#0a0f3c] rounded-[22px] font-black text-[11px] uppercase shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all">
            ...
            myRequests.length > 0 ? myRequests.map(post => (
              ...
              <div className="text-center py-20 opacity-30 font-black uppercase text-xs tracking-widest">لا توجد طلبات نشطة</div>
            )
          ) : (
             communityPlayers.map(player => (
               <div key={player.id} className="bg-white/5 border border-white/10 rounded-[35px] p-6 flex items-center justify-between backdrop-blur-xl shadow-2xl group hover:border-cyan-500/30 transition-all">
                  ...
                  <button 
                    ...
                    className="px-6 py-3 bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 rounded-2xl text-[10px] font-black uppercase hover:bg-cyan-500 hover:text-[#0a0f3c] transition-all active:scale-90 flex items-center gap-2"
                  >
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-xl">
            <div className="bg-[#0a0f3c]/90 border border-white/10 w-full max-w-sm rounded-[45px] p-10 space-y-8 shadow-2xl animate-in zoom-in duration-300">
              ...
              <button onClick={handleCreatePost} disabled={isSubmitting} className="w-full py-5.5 bg-cyan-500 text-[#0a0f3c] rounded-[26px] font-[1000] uppercase text-xs flex items-center justify-center gap-3 active:scale-95 transition-all">
