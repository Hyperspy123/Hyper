...
export default function Faz3a() {
  ...
  const [isJoining, setIsJoining] = useState<string | null>(null); // لتتبع حالة الانضمام من القائمة العامة
  ...
  useEffect(() => {
    fetchData();
    // اشتراك Realtime لتحديث عدد اللاعبين فوراً عند انضمام أي شخص
  ...
  // 🔥 دالة الانضمام الذكية من القائمة العامة
  const handleJoinFromPublic = async (postId: string) => {
    ...
    try {
      ...
      if (success) {
        toast.success("كفو! أبشر بالفزعة تم تسجيلك بنجاح 🔥");
        setActiveTab('my_requests'); // نقله ليرى الفزعة في "الشباب طالبينك"
      } else {
      ...
      <main className="p-6 max-w-md mx-auto space-y-8 pt-24">
        ...
        <div className="flex bg-white/5 p-1.5 rounded-[24px] border border-white/10 backdrop-blur-3xl gap-1.5">
            {[{ id: 'others_requests', label: 'فزعات عامة' }, { id: 'my_requests', label: 'طلباتي' }, { id: 'community', label: 'المجتمع' }].map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex-1 py-3.5 rounded-[18px] text-[10px] font-black uppercase transition-all ${activeTab === tab.id ? 'bg-cyan-500 text-[#0a0f3c] shadow-lg shadow-cyan-400/20 scale-105' : 'text-gray-500'}`}>{tab.label}</button>
            ))}
        </div>
              ...
              return (
                <div key={post.id} className={`relative bg-white/5 border rounded-[35px] p-7 space-y-5 backdrop-blur-2xl transition-all duration-500 ${isMyPost ? 'border-cyan-500/50' : 'border-white/10'} ${isFull ? 'opacity-50' : ''}`}>
                  <div className="flex justify-between items-start">
                    ...
                    {isFull ? (
                      <div className="bg-gray-500/20 border border-gray-500/30 text-gray-400 px-4 py-1.5 rounded-full text-[10px] font-black italic uppercase flex items-center gap-1">
                        <CheckCircle2 size={12} /> مكتملة
                      </div>
                    ) : (
                      <div className="bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 px-4 py-1.5 rounded-full text-[10px] font-black italic uppercase tracking-tighter">ناقص {post.missing_players}</div>
                    )}
                  </div>
                  ...
                  {!isMyPost && !isFull && (
                    <button 
                      onClick={() => handleJoinFromPublic(post.id)}
                      disabled={isJoining === post.id}
                      className="w-full py-4.5 bg-white text-[#0a0f3c] rounded-[22px] font-black text-[11px] uppercase shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2"
                    >
                  ...
                  {isFull && (
                    <div className="w-full py-4 bg-white/5 border border-white/10 rounded-[22px] text-center text-gray-500 text-[10px] font-black uppercase italic">تم تلبية هذه الفزعة 🛡️</div>
                  )}
                </div>
              );
            })
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-4">
                 ...
                 {joinedRequests.map(post => (
                    <div key={post.id} className="relative bg-gradient-to-br from-yellow-500/10 to-transparent border border-yellow-500/30 rounded-[35px] p-7 space-y-5 shadow-2xl">
                       <div className="absolute top-4 left-4 bg-yellow-500 text-[#0a0f3c] px-3 py-1 rounded-full text-[8px] font-black uppercase italic rotate-[-5deg]">الشباب طالبينك</div>
                       <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-2xl bg-yellow-500/10 flex items-center justify-center text-yellow-500 font-black">{post.profiles?.first_name?.[0]}</div>
                          <div>
                             <h4 className="font-black text-sm text-white italic">داعي الفزعة: <span className="text-yellow-500">{post.profiles?.first_name}</span></h4>
                       ...
                       <div className="flex items-center justify-between gap-3 pt-2">
                          ...
                          <button onClick={() => window.open(`https://wa.me/9665XXXXXXXX`)} className="flex-1 py-3 bg-green-500 text-[#0a0f3c] rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 shadow-lg"><MessageCircle size={14} /> تواصل واتساب</button>
                       </div>
                    </div>
                 ))}
              </div>
             ...
             communityPlayers.map(player => (
               <div key={player.id} className="bg-white/5 border border-white/10 rounded-[35px] p-6 flex items-center justify-between">
                  <div className="flex items-center gap-4 text-right">
                    <div className="w-14 h-14 rounded-2xl bg-cyan-500/5 border border-white/5 flex items-center justify-center font-black text-cyan-400 text-lg">{player.first_name?.[0]}</div>
                    <div>
                  ...
                  <button disabled={invitingId === player.id} onClick={() => handleSendInvite(player.id, player.first_name)} className="px-6 py-3 bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 rounded-2xl text-[10px] font-black uppercase flex items-center gap-2">
                    {invitingId === player.id ? <Loader2 className="animate-spin" size={14} /> : <><Send size={14} /> دعوة</>}
                  </button>
               </div>
             ))
          )}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-xl animate-in fade-in">
            <div className="bg-[#0a0f3c]/90 border border-white/10 w-full max-w-sm rounded-[45px] p-10 space-y-8 shadow-2xl text-right" dir="rtl">
              <div className="flex justify-between items-center">
                <h3 className="text-3xl font-[1000] italic text-cyan-400 uppercase">إنشاء فزعة</h3>
              ...
              <div className="space-y-5">
                <label className="text-[10px] font-black text-gray-500 uppercase px-2 italic">كم بطل ناقصك؟</label>
                <div className="flex gap-3">
                  {[1, 2, 3].map(num => (
                    <button key={num} onClick={() => setMissingPlayers(num)} className={`flex-1 py-5 rounded-[22px] font-[1000] text-lg border transition-all ${missingPlayers === num ? 'bg-cyan-500 border-cyan-400 text-[#0a0f3c] shadow-xl' : 'bg-white/5 border-white/10 text-gray-400'}`}>{num}</button>
                  ))}
                </div>
              </div>
              <button onClick={handleCreatePost} disabled={isSubmitting} className="w-full py-5.5 bg-cyan-500 text-[#0a0f3c] rounded-[26px] font-[1000] uppercase text-xs flex items-center justify-center gap-3">
