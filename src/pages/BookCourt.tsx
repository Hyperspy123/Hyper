...
export default function BookCourt() {
  ...
  // تحسين جلب الأوقات المحجوزة لتكون متوافقة مع المناطق الزمنية
  const fetchBookedSlots = useCallback(async (date: string) => {
    if (!cleanId || !date) return;
    
    // نحدد بداية ونهاية اليوم بتوقيت السعودية لفلترة دقيقة
    ...
    if (!error && data) {
      const times = data.map(b => {
        ...
        // استخراج الوقت بتنسيق HH:mm ثابت للمقارنة مع timeSlots
  ...
  // 🔥 الدالة المعدلة لضمان مطابقة الوقت 100% مع قاعدة البيانات
  const handleConfirm = async () => {
    ...
    try {
      // بناء التاريخ والوقت مع التصريح بالمنطقة الزمنية (+03:00 للسعودية)
      ...
      const { error: bookingError } = await supabase.from('bookings').insert([{ 
        court_id: cleanId, 
        user_id: user.id, 
        start_time: startTimeISO, // نرسل الوقت المحلي مع الـ Offset
      ...
      <div className="px-6 mt-4 flex justify-end">
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all duration-500 ${viewerCount > 1 ? 'bg-cyan-500/10 border-cyan-500/30' : 'bg-white/5 border-white/10 opacity-60'}`}>
          ...
          <span className="text-[10px] font-black uppercase text-cyan-400">
            {viewerCount} أشخاص يشاهدون الآن
          </span>
      ...
      <main className="px-6 max-w-md mx-auto space-y-8 text-right">
        ...
        <div className="bg-[#14224d] rounded-[35px] p-6 border border-white/10 flex items-center gap-5 shadow-2xl relative overflow-hidden group">
          ...
          <div className="text-right z-10">
            ...
            <span className="bg-cyan-500/10 text-cyan-400 px-3 py-1 rounded-full font-black text-xs border border-cyan-500/20">
              {court?.price_per_hour} SAR / ساعة
            </span>
          </div>
        </div>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {durations.map((d) => (
              <button key={d.value} onClick={() => setDuration(d.value)} className={`py-4 rounded-2xl border-2 font-black text-[11px] transition-all duration-300 ${duration === d.value ? 'bg-cyan-500 border-cyan-400 text-[#0a0f3c] shadow-[0_0_20px_rgba(34,211,238,0.3)]' : 'bg-[#14224d] border-white/5 text-gray-400'}`}>{d.label}</button>
            ))}
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            {getDateButtons().map((day) => (
              <button key={day.iso} onClick={() => setSelectedDate(day.iso)} className={`flex flex-col items-center justify-center aspect-square rounded-[28px] border-2 transition-all duration-300 ${selectedDate === day.iso ? 'bg-cyan-500 border-cyan-400 text-[#0a0f3c] shadow-lg shadow-cyan-500/20' : 'bg-[#14224d] border-white/5 text-gray-500 hover:border-white/20'}`}>
              ...
              return (
                <button 
                  ...
                  className={`py-5 rounded-2xl border-2 font-black text-xs transition-all relative overflow-hidden flex items-center justify-center ${
                    isBooked 
                    ? 'bg-gray-950 border-white/5 text-gray-700 cursor-not-allowed opacity-40' 
                    : selectedTime === slot.id 
                      ? 'bg-cyan-500 border-cyan-400 text-[#0a0f3c] shadow-lg shadow-cyan-500/20' 
                      : 'bg-[#14224d] border-white/5 text-gray-400 hover:border-cyan-500/30'
                  }`}
                >
        ...
        <button 
          ...
          className="w-full py-6 bg-cyan-500 text-[#0a0f3c] rounded-[30px] font-[1000] text-xl shadow-[0_20px_40px_rgba(34,211,238,0.2)] flex items-center justify-center gap-3 active:scale-[0.97] disabled:opacity-20 disabled:grayscale transition-all mt-6 uppercase italic tracking-tighter"
        >
