...
export default function BookCourt() {
  ...
  const getCleanId = (rawId: string | undefined) => {
    if (!rawId) return '';
    return rawId.replace(/%22/g, '').replace(/%27/g, '').replace(/['"]+/g, '').trim();
  };
  ...
  if (loading) return <div className="min-h-screen bg-[#0a0f3c] flex items-center justify-center text-cyan-400 font-black italic">LOADING...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#0a0f3c] text-white font-sans pb-24" dir="rtl">
      ...
      <main className="px-6 max-w-md mx-auto space-y-8 text-right">
        {/* Court Card */}
        <div className="bg-[#14224d] rounded-[32px] p-5 border border-white/5 flex items-center gap-5">
          <img src={court?.image_url || court?.image} className="w-20 h-20 rounded-2xl object-cover border border-cyan-400/20" />
          <div className="text-right">
            <h2 className="text-xl font-black mb-1">{court?.name || "ملعب بادل"}</h2>
            <span className="text-cyan-400 font-black text-sm">{court?.price_per_hour || 250} SAR</span>
          </div>
        </div>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {timeSlots.map((slot) => (
              <button key={slot.id} onClick={() => setSelectedTime(slot.id)} className={`py-4 rounded-2xl border-2 font-black text-[11px] transition-all ${selectedTime === slot.id ? 'bg-cyan-500 border-cyan-400 text-[#0a0f3c]' : 'bg-[#14224d] border-white/5 text-gray-400'}`}>{slot.label}</button>
            ))}
          </div>
        </section>
